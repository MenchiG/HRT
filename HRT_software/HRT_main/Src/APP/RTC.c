...
u8 const *WEEK_STR[] = {"7", "1", "2", "3", "4", "5", "6"};
...
void Time_Regulate(RTC_InitTypeDef *tm)
{
  if(GPS_Data.GPS_VA)
  {
  	tm->tm_year=GPS_Data.Year+2000;
	tm->tm_mon=GPS_Data.Mouth;
	tm->tm_mday=GPS_Data.Day;
	tm->tm_hour=GPS_Data.Hour;
	tm->tm_min=GPS_Data.Min;
	tm->tm_sec=GPS_Data.Sec;
  }
  else
  {
  	tm->tm_year=2012;
 ...
 */
void GregorianDay(RTC_InitTypeDef * tm)
{
	int leapsToDate;
	int lastYear;
	int day;
 ...
 */
unsigned int mktimev(RTC_InitTypeDef *tm)
{
...
void to_tm(unsigned int tim, RTC_InitTypeDef * tm)
{
