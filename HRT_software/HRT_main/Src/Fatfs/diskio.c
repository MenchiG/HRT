...
/* - for SPI2 and full-speed APB1: 72MHz/4 */
...
__ramfunc void disk_timerproc (void)
{
	static DWORD pv;
	DWORD ns;
	BYTE n, s;
...
{
	DWORD time;
	time = (((systmtime.tm_year-1980) &0x7f)<< 25) | ((systmtime.tm_mon + 2 & 0x0f)<< 21) | (((systmtime.tm_mday) &0x1F)<< 16) |
		   ((systmtime.tm_hour&0x1F) << 11) | ((systmtime.tm_min&0x3F) << 5) | (((systmtime.tm_sec/2)&0x0F) << 0);
	return time;
}
